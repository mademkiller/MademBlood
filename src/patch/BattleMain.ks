...
tf.HARAKIRI_L = 0;//自決フラグ.L
tf.HARAKIRI_R = 0;//自決フラグ.R
...
dm("("+i+")chNO:"+gf.enemy[tf.EncountL[0][0]].dev[i].id);
		switch(gf.enemy[tf.EncountL[0][0]].dev[i].id){
			case 2171://シンモラ２
			case 2093://真オーディン２
				counter = 7;bgmNo = 'td10_op';
				break;

			case 2170://シンモラ
				if(counter < 6){counter = 6;bgmNo = 'bgm26';}
				break;
			...
			case 2150:
				...
				if(counter <= 4 && f.game_mode == 1){counter = 4;bgmNo = 'td09_bgm25';}
				break;
			...
			case 2160:
				...
				if(counter <= 3 && f.game_mode == 1){counter = 3;bgmNo = 'td09_bgm25';}
				break;
			...
			case 787://最上位ドラゴン系（過去作ＢＧＭ）
				if(counter < 0.8){counter = 0.8;bgmNo = 'td09_bgm25';}
				break;

			case 1700://四聖獣系（過去作ＢＧＭ）
			case 1701:
			case 1702:
			case 1703:
				if(counter < 0.7){counter = 0.7;bgmNo = 'td09_bgm24';}
				break;

			case 1750://天使系（過去作ＢＧＭ）
			case 1760:
			case 1770:
			case 1780:
			case 1790:
			...
			case 1850:
			case 1860:
				if(counter < 0.6){counter = 0.6;bgmNo = 'td09_bgm19';}
				break;

			case 785://上位ドラゴン系（過去作ＢＧＭ）
			case 786:
				if(counter < 0.5){counter = 0.5;bgmNo = 'td09_bgm17';}
				break;

			case 900://禁獣系（過去作ＢＧＭ）
			case 910:
				if(counter < 0.4){counter = 0.4;bgmNo = 'td09_bgm26';}
				break;

			default ://通常
				//バトルロイヤルモード・ステージ４通常戦闘（過去作ＢＧＭ）
				if(f.game_mode == 1 && f.BGM_ID == 92 && counter < 0.06){counter = 0.06;bgmNo = 'td09_bgm17';}
				//バトルロイヤルモード・ステージ３通常戦闘（過去作ＢＧＭ）
				if(f.game_mode == 1 && f.BGM_ID == 93 && counter < 0.05){counter = 0.05;bgmNo = 'td09_bgm19';}
				//バトルロイヤルモード・ステージ２通常戦闘（過去作ＢＧＭ）
				if(f.game_mode == 1 && f.BGM_ID == 91 && counter < 0.04){counter = 0.04;bgmNo = 'td07_bgm14';}
				//バトルロイヤルモード・ステージ１通常戦闘（過去作ＢＧＭ）
				if(f.game_mode == 1 && f.BGM_ID == 90 && counter < 0.03){counter = 0.03;bgmNo = 'td07_bgm14';}

				//ロウルート８章通常戦闘（過去作ＢＧＭ）
				if(f.route=='law' && f.game_mode !=1 && counter < 0.02){counter = 0.02;bgmNo = 'td09_bgm22';}
				//カオスルート８章通常戦闘（過去作ＢＧＭ）
				if(f.route=='chaos' && f.game_mode !=1  && counter < 0.01){counter = 0.01;bgmNo = 'td09_bgm22';}
...
for(var i=0;i<6;i++){
	...
	//フィールドステータス表示
	if(tf.Encounter == 0){
		tf.viewFlagsNumber = tf.enemySelectNumber;
...
dm("敵<-自="+tf.allyTarget+"/敵->自"+tf.enemyTarget);
	...
	//戦術スキル絶対治療
	if(tf.Medical_L == 1){tf.defenseL.絶対 = 1;dm("tf.defenseL.絶対="+tf.defenseL.絶対);}
...
dm("L.遠隔="+tf.offenseL.遠隔+"/時間:"+tf.day_time+"/L.夜戦:"+tf.offenseL.夜戦);
		 tf.OriginalTargetL = void;
	     if(tf.offenseL.遠隔 == 1 && tf.day_time == 0 && tf.offenseL.夜戦 == 1 && tf.defenseR.遠隔 == 0){dm("L.遠隔(夜)");}//遠隔・時間(夜)・夜戦判定
	else if(tf.offenseL.遠隔 == 1 && tf.day_time == 1 && tf.defenseR.遠隔 == 0){						 dm("L.遠隔(昼)");}//遠隔・時間(昼)
	else if(tf.offenseL.遠隔 == 1 && tf.day_time == 0 && tf.offenseL.夜戦 == 1 && tf.offenseL.次元 == 1){dm("L.次元(夜)");}//遠隔・時間(夜)・夜戦判定・次元判定
	else if(tf.offenseL.遠隔 == 1 && tf.day_time == 1 && tf.offenseL.次元 == 1){						 dm("L.次元(昼)");}//遠隔・時間(昼)・次元判定
	else if(tf.offenseL.側面 == 1 && tf.defenseR.側面 == 0){											 dm("L.側面攻撃");}//側面攻撃・側面無効判定
	else{dm("L.通常攻撃");
		//ターゲットスキャン(右)
		tf.enemyTargetOriginal = tf.enemyTarget;
		dm("enemyTarget="+tf.enemyTarget);
		if(tf.enemyTarget > 2){
			var tmpTarget = tf.enemyTarget - 3;
			targetScan = battleFuncsView.frontSearch('right');
			dm("R.前衛生存中:"+targetScan[tmpTarget]);
			if(targetScan[tmpTarget] == 1){//直前衛生存中
				tf.enemyTarget     = tmpTarget;
				tf.OriginalTargetL = tmpTarget;
				dm("L.新ﾀｰｹﾞｯﾄ:"+tf.enemyTarget);
			}else{
				var FrontCheck=0;
				for(var i=0;i<targetScan.count;i++){
					if(targetScan[i] == 1){//生存中前衛検索
						tf.enemyTarget     = i;
						tf.OriginalTargetL = i;
						dm("LF.新ﾀｰｹﾞｯﾄ["+i+"]:"+tf.enemyTarget);
						FrontCheck=1;
						break;
					}
				}
				if(FrontCheck==0){
					for(var i=3;i<3;i++){
						if(tf.leftParty[i].HP.now > 0 && tf.leftParty[i].id != 0){
							tf.enemyTarget     = i;
							tf.OriginalTargetL = i;
							dm("LB.新ﾀｰｹﾞｯﾄ["+i+"]:"+tf.enemyTarget);
							break;
						}
					}
				}
			}
		}else{tf.OriginalTargetL = tf.enemyTarget;}
	}
	if(tf.OriginalTargetL == void){tf.OriginalTargetL = tf.enemyTarget;}

dm("R.遠隔="+tf.offenseR.遠隔+"/時間:"+tf.day_time+"/R.夜戦:"+tf.offenseR.夜戦);
//後列攻撃判定(夜無効-夜戦以外)
		 tf.OriginalTargetR = void;
	     if(tf.offenseR.遠隔 == 1 && tf.day_time == 0 && tf.offenseR.夜戦 == 1 && tf.defenseL.遠隔 == 0){dm("R.遠隔(夜)");}//遠隔・時間(夜)・夜戦判定
	else if(tf.offenseR.遠隔 == 1 && tf.day_time == 1 && tf.defenseL.遠隔 == 0){						 dm("R.遠隔(昼)");}//遠隔・時間(昼)
	else if(tf.offenseR.遠隔 == 1 && tf.day_time == 0 && tf.offenseR.夜戦 == 1 && tf.offenseR.次元 == 1){dm("R.次元(夜)");}//遠隔・時間(夜)・夜戦判定・次元判定
	else if(tf.offenseR.遠隔 == 1 && tf.day_time == 1 && tf.offenseR.次元 == 1){						 dm("R.次元(昼)");}//遠隔・時間(昼)・次元判定
	else if(tf.offenseR.側面 == 1 && tf.defenseL.側面 == 0){											 dm("R.側面攻撃");}//側面攻撃・側面無効判定
	else{dm("R.通常攻撃");
		//ターゲットスキャン(左)
		tf.allyTargetOriginal = tf.allyTarget;
		if(tf.allyTarget > 2){
			var tmpTarget = tf.allyTarget - 3;
			targetScan = battleFuncsView.frontSearch('left');
			dm("L.前衛生存中:"+targetScan[tmpTarget]);
			if(targetScan[tmpTarget] == 1){//直前衛生存中
				tf.allyTarget      = tmpTarget;
				tf.OriginalTargetR = tmpTarget;
				dm("R.新ﾀｰｹﾞｯﾄ:"+tf.allyTarget);
			}else{
				var FrontCheck=0;
				for(var i=0;i<targetScan.count;i++){
					if(targetScan[i] == 1){//生存中前衛検索
						tf.allyTarget      = i;
						tf.OriginalTargetR = i;
						dm("RF.新ﾀｰｹﾞｯﾄ["+i+"]:"+tf.allyTarget);
						FrontCheck=1;
						break;
					}
				}
				if(FrontCheck==0){
					for(var i=3;i<3;i++){
						if(tf.rightParty[i].HP.now > 0 && tf.rightParty[i].id != 0){
							tf.allyTarget     = i;
							tf.OriginalTargetR = i;
							dm("RB.新ﾀｰｹﾞｯﾄ["+i+"]:"+tf.allyTarget);
							break;
						}
					}
				}
			}
		}else{tf.OriginalTargetR = tf.allyTarget;}
	}
	if(tf.OriginalTargetR == void){tf.OriginalTargetR = tf.allyTarget;}
...
dm("ATカウント"+tf.attackCount);
		if(tf.attackCount == 0){
			tf.OriginalTarget=tf.allyTarget;dm("(0)OriginalTarget="+tf.OriginalTarget);
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'貫通攻撃','right');if(SkillValue[0] == 1){tf.offenseR.貫通 = 2;}
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'扇形攻撃','right');if(SkillValue[0] == 1){tf.offenseR.扇形 = 3;}
				if(tf.offenseR.貫通 > 0 && tf.offenseR.扇形 > 0){tf.offenseR.十字 = 1;}
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'十字攻撃','right');if(SkillValue[0] == 1){tf.offenseR.十字 = 1;tf.offenseR.貫通 = 2;tf.offenseR.扇形 = 3;}
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'全域攻撃','right');if(SkillValue[0] == 1){tf.offenseR.全域 = 2;tf.offenseR.扇形 = 3;}
...
dm("1現在ﾀｹﾞ:"+tf.allyTarget+" /防御:"+tf.defender);
		if(tf.offenseR.全域 > 0 && tf.defenseL.全域 == 1){tf.offenseR.全域 = 0;tf.offenseR.扇形 = 0;}//敵に全域無効があったら
dm("2現在ﾀｹﾞ:"+tf.allyTarget+" /防御:"+tf.defender);
		if(tf.offenseR.扇形 == 3 && (tf.defenseL.扇形 == 0 || tf.defenseL.十字 == 0)){tf.allyTarget = tf.OriginalTargetR;tf.defenseL.防御 = 0;tf.defenseL.前進 = 0;}//扇形有効なら防御陣形・前進防御時のﾀｹﾞ移動を修正して無効に
dm("3現在ﾀｹﾞ:"+tf.allyTarget+" /防御:"+tf.defender);
		if(tf.offenseR.全域 > 1 && tf.defenseL.全域 == 0){
			tf.defenseL.扇形 = 0;//全域だったら扇形無効を無効
			tf.defenseL.十字 = 0;//全域だったら十字無効を無効
			//敵に全域無効がなかったら強制的にﾀｹﾞ移動
			if(tf.allyTargetOriginal !== void){tf.OriginalTarget=tf.allyTargetOriginal;}else{tf.OriginalTarget=tf.allyTarget;dm("(1)OriginalTarget="+tf.OriginalTarget);}
dm("4現在ﾀｹﾞ:"+tf.allyTarget+" /防御:"+tf.defender);
			var foreTmp=0;
			var backTmp=0;
			...
			//後衛が壊滅してたら前列へﾀｹﾞ移動
dm("5現在ﾀｹﾞ:"+tf.allyTarget+" /防御:"+tf.defender);
			if(backTmp == 0){
				tf.offenseR.全域 = 1;
				tf.allyTarget = 0;
			}
dm("6現在ﾀｹﾞ:"+tf.allyTarget+" /防御:"+tf.defender);
		}
		//扇形攻撃(タゲ選定)
...
dm("L.防御="+tf.defenseL.防御);
			...
			//HP減算
			if(tf.leftWall > 0 && tf.HARAKIRI_L == 0){
			...
			if(tf.rightWall > 0 && tf.HARAKIRI_R == 0){
...
dm("endR.全域:"+tf.offenseR.全域+"/R.扇形"+tf.offenseR.扇形);
			if(tf.offenseR.全域 == 2 && tf.offenseR.扇形 == 0){tf.offenseR.全域--;tf.offenseR.扇形 = 3;tf.attackCount = 3;tf.allyTarget = tf.clothAttack + 3;tf.OriginalTargetR = tf.allyTarget;}
...
dm("R.十字:"+tf.offenseR.十字+"R.扇形:"+tf.offenseR.扇形);
			if(tf.offenseR.十字 == 1 && tf.offenseR.扇形 == 0){tf.allyTarget = tf.OriginalTargetR;tf.offenseR.十字 = 0;}//十字用
...
dm("R.扇形:"+tf.offenseR.扇形+"/R.貫通:"+tf.offenseR.貫通);
	...
	if(tf.attSide == 'L'){
		...
		//攻撃スキルチェック
			...
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'扇形攻撃','left');if(SkillValue[0] == 1){tf.offenseL.扇形 = 3;}
			if(tf.offenseL.貫通 > 0 && tf.offenseL.扇形 > 0){tf.offenseL.十字 = 1;}
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'十字攻撃','left');if(SkillValue[0] == 1){tf.offenseL.十字 = 1;tf.offenseL.貫通 = 2;tf.offenseL.扇形 = 3;}
			var SkillValue = battleFuncSkillSet.searchSkill(tf.attacker,'全域攻撃','left');if(SkillValue[0] == 1){tf.offenseL.全域 = 2;tf.offenseL.扇形 = 3;}
...
dm("1現在ﾀｹﾞ:"+tf.enemyTarget+" /防御:"+tf.OriginalTargetL);
		if(tf.offenseL.全域 > 0 && tf.defenseR.全域 == 1){tf.offenseL.全域 = 0;tf.offenseL.扇形 = 0;}//敵に全域無効があったら
dm("2現在ﾀｹﾞ:"+tf.enemyTarget+" /防御:"+tf.OriginalTargetL);
		if(tf.offenseL.扇形 == 3 && (tf.defenseR.扇形 == 0 || tf.defenseR.十字 == 0)){tf.enemyTarget = tf.OriginalTargetL;tf.defenseR.防御 = 0;tf.defenseR.前進 = 0;}//扇形有効なら防御陣形・前進防御時のﾀｹﾞ移動を修正して無効に
dm("3現在ﾀｹﾞ:"+tf.enemyTarget+" /防御:"+tf.OriginalTargetL);
...
dm("4現在ﾀｹﾞ:"+tf.enemyTarget+" /防御:"+tf.OriginalTargetL);
			var foreTmp=0;
			var backTmp=0;
...
dm("5現在ﾀｹﾞ:"+tf.enemyTarget+" /防御:"+tf.defender);
			//後衛が壊滅してたら前列へﾀｹﾞ移動
			if(backTmp == 0){
				tf.offenseL.全域 = 1;
				tf.enemyTarget = 0;
			}
dm("6現在ﾀｹﾞ:"+tf.enemyTarget+" /防御:"+tf.defender);
		}
		//扇形攻撃(タゲ選定)
...
dm("E.HP"+tf.rightParty[tf.defender].HP.now);
			...
			//扇形処理(次タゲ選定)
			if((tf.offenseL.扇形 > 0 && tf.defenseR.扇形 == 0 && tf.defenseR.十字 == 0) || (tf.offenseL.扇形 > 0 && tf.offenseL.全域 > 0)){
...
dm("endL.全域:"+tf.offenseL.全域+"/L.扇形"+tf.offenseL.扇形);
			if(tf.offenseL.全域 == 2 && tf.offenseL.扇形 == 0){tf.offenseL.全域--;tf.offenseL.扇形 = 3;tf.attackCount = 3;tf.enemyTarget = tf.clothAttack + 3;tf.OriginalTargetL = tf.enemyTarget;}
...
dm("L.十字:"+tf.offenseL.十字+"L.扇形:"+tf.offenseL.扇形);
			if(tf.offenseL.十字 == 1 && tf.offenseL.扇形 == 0){tf.enemyTarget = tf.OriginalTargetL;tf.offenseL.十字 = 0;}//十字用
...
//	battleFuncsView.unitStatusWindowDraw();	//ユニットパラメータ表示
	...
	tf.mapReviseCarryR = 0;//兵士運搬R
	tf.mapReviseCarryL = 0;//兵士運搬L
	tf.defenseL.回避 = 0;//イベイド.L
	tf.defenseR.回避 = 0;//イベイド.R
	tf.defenseL.耐性 = 0;
	tf.defenseR.耐性 = 0;
	tf.HARAKIRI_L = 0;//自決フラグ.L
	tf.HARAKIRI_R = 0;//自決フラグ.R
...
[call storage="battleInit.ks" target=*武将情報の書き戻し処理]
			...
			for(var n=0;n<6;n++){
				if(tf.rightParty[n].id != 0){
					if(tf.rightParty[n].id == gf.char[i].id){
						...
						if(tf.rightParty[n].valor   > 100){tf.rightParty[n].valor   = 100;}
...
[if exp="tf.Encounter == 0"]
	...
	[endscript]
	[stopshakes layer=all]
	;▲↑原因はまったく不明だが、このシェイク止めを入れておかないと、この下の行の下（[waitshakes canskip=false]より後ろ）へ処理が流れていかない不具合がある。
...
//戦術表示テスト
//	tf.test = calc.viewTacticsSkillList(f.used.party[tf.allysSelectNumber].division);
...
dm("部隊ｶｳﾝﾀ:"+statcout);
		do{
			...
			if(tf.EncountL[i][n] !== void){
				dm("["+gf.enemy[tf.EncountL[i][n]].name+"/"+gf.enemy[tf.EncountL[i][n]].id+"] の全滅ﾌﾗｸﾞは、"+gf.enemy[tf.EncountL[i][n]].status+" だッ!!");
				if(gf.enemy[tf.EncountL[i][n]].status == 0){//全滅ならマップから削除
					if(tf.winners == 0){
						gf.map[tf.EncountM[i]].post.division.erase(n);
						dm("--部隊削除 削除ｶｳﾝﾄ:"+n+" 削除箇所"+gf.map[tf.EncountM[i]].name);
						n++;
					}
