...
[if exp="sf.クリア情報[0]>=1"]
	...
	[elsif exp="f.game_mode==2"][pimage layer=0 page=back storage="mode_bt03" dx=83 dy=131 sx=288 sy=0 sw=144 sh=26][endif]

	[if exp="f.extra_mode==1"][locate x=83 y=159][button graphic="mode_bt04" exp="f.extra_mode=0" target=*start2 clickse="se9000_システムＳＥ４" clicksebuf=0]
	[elsif exp="f.game_mode==3"][pimage layer=0 page=back storage="mode_bt04" dx=83 dy=159 sx=288 sy=0 sw=144 sh=26][endif]
[endif]
...
;[locate x=236 y=246]クリア時の兵種状態を維持してゲームプレイ。（クリア時に悪堕ちなら悪からスタート）
